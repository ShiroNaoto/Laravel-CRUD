    ...
    <div class="sidebar">
      ...
      <nav class="mt-2">
        <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
          ...
          <li class="nav-item">
            <a href="{{ route('user.supticket')}}" class="nav-link">
                ...
                <table id="table1" class="table table-bordered table-hover">
                  ...
                  <tbody>
                    @foreach($tickets as $ticket)
                      @if(Auth::user()->id == $ticket->user_id && in_array($ticket->state, ['Pending', 'Processing']))
                    <tr>
                        ...
                        <td>
                          ...
                          <div class="dropdown-menu" role="menu">
                            <a class="dropdown-item" data-toggle="modal" data-target="#viewTicket{{ $ticket->id }}" >View</a>
      ...
      <div class="chart tab-pane" id="process">
         <div class="card-body">
                <table id="table2" class="table table-bordered table-hover">
                  ...
                  <tbody>
                    @foreach($tickets as $ticket)
                      @if(Auth::user()->id == $ticket->user_id && $ticket->state === 'Resolved')
                    <tr>
                        <td class="text-center sorting_1">
                        @switch($ticket->state)
                          @case('Pending')  
...
<div class="modal fade" id="destroyTicket{{$ticket->id}}">
    <div class="modal-dialog">
    <form method="POST" action="{{ route('destroyTickethome')}}">
               @csrf
               @method('DELETE')
               <input type="hidden" name="id" value="{{ $ticket->id }}">
